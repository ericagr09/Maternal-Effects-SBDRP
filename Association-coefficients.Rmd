...
#### (code taken from Family_Size_mirror repository, as well as data. Added notes for my own personal understanding within code chunks.)

Read in data and prep data.

```{r data}
load("kfinal1000.RData")
indiv_covars <- read.csv("individual_covariates.csv")
...
Select pairs to from which to determine affiliations, affiliated pairs are calculated between all individuals with at least 2 years postweaning of 35 relocations and 20 sightings in which group associations were recorded and who have been genotyped
...
save(masked_network, ai_affil_rand, file = "affiliation_networks.RData")
```
